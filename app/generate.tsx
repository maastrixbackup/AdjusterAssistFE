...
export default function GenerateScreen() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [statusData, draftsData] = await Promise.all([
        ...
        getDraftsByFile(token, 1)
      ]);
      setStatus(statusData);
      setRecentDrafts(draftsData.slice(0, 3));
    } catch (err) {
  ...
  const onGenerate = useCallback(async () => {
    ...
    if (!request.trim()) {
      Toast.show({ type: 'error', text1: 'Content Required', text2: 'Describe the scenario to generate a draft.' });
      return;
    }
    ...
    try {
      ...
      Toast.show({ type: 'error', text1: 'Generation Failed', text2: error.message || "API Error" });
    } finally {
          ...
          <View style={styles.tabContainer}>
            {outputModes.map((mode) => {
              ...
              return (
                ...
                >
                  <Text style={[styles.tabText, isActive && styles.tabTextActive]}>{mode}</Text>
                </Pressable>
              );
            })}
          </View>
            ...
            <View style={styles.fieldGroup}>
              <View style={styles.fieldHeader}>
                <View style={styles.iconCircle}>
                  <MaterialCommunityIcons name="text-box-search-outline" size={16} color="#0F4C9C" />
                </View>
          ...
          <View style={styles.historyList}>
            {recentDrafts.map((item) => (
              <View key={item.id} style={styles.historyItem}>
                <View style={[styles.historyIconBox, { backgroundColor: item.type === 'email' ? '#E0F2FE' : '#F8FAFC' }]}>
                  <Ionicons 
                    name={item.type === "email" ? "mail-unread-outline" : "document-text-outline"} 
                    size={20} 
                    color="#0F4C9C" 
                  />
                </View>
                <View style={styles.historyContent}>
                  ...
                  <Text style={styles.historyText} numberOfLines={1}>{item.content || "Processing draft..."}</Text>
                </View>
...
const styles = StyleSheet.create({
  ...
  navTitle: { fontSize: 18, fontWeight: "800", color: "#0F172A", letterSpacing: -0.5 },
  ...
  creditValue: { color: '#475569', fontSize: 13, fontWeight: '700' },
  scrollView: { flex: 1 },
  scrollContent: { padding: 20, paddingBottom: 40 },
  sectionLabel: { fontSize: 11, fontWeight: "800", color: "#94A3B8", textTransform: 'uppercase', letterSpacing: 1.5, marginBottom: 14, marginLeft: 4 },
  ...
  tabItemActive: { backgroundColor: "#FFF", shadowColor: "#0F172A", shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.12, shadowRadius: 8, elevation: 4 },
  ...
  mainTextInput: { fontSize: 16, color: "#334155", minHeight: 120, textAlignVertical: 'top', lineHeight: 22 },
  cardDivider: { height: 1, backgroundColor: "#F1F5F9", marginVertical: 20, marginHorizontal: -10 },
  subTextInput: { fontSize: 15, color: "#475569", minHeight: 80, textAlignVertical: 'top', lineHeight: 20 },
  generateBtn: { marginTop: 30, borderRadius: 20, overflow: 'hidden', shadowColor: "#0F4C9C", shadowOffset: { width: 0, height: 10 }, shadowOpacity: 0.25, shadowRadius: 15, elevation: 8 },
  gradientBtn: { height: 64, flexDirection: 'row', alignItems: "center", justifyContent: "center", gap: 12 },
  btnText: { color: "#FFFFFF", fontSize: 17, fontWeight: "800", letterSpacing: 0.5 },
  historyHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginTop: 40, marginBottom: 20 },
  historyTitle: { fontSize: 18, fontWeight: "800", color: "#0F172A", letterSpacing: -0.3 },
  historyList: { gap: 14 },
  historyItem: { 
    flexDirection: 'row', 
    alignItems: 'center', 
  ...
  historyIconBox: { width: 48, height: 48, borderRadius: 14, alignItems: 'center', justifyContent: 'center' },
  historyContent: { flex: 1, marginLeft: 16 },
  historyTypeTag: { fontSize: 10, fontWeight: '900', color: '#94A3B8', letterSpacing: 1, marginBottom: 4 },
